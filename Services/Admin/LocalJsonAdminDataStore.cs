...
public sealed class LocalJsonAdminDataStore : IAdminDataStore
{
    ...
    {
        var data = await ReadDataAsync(cancellationToken);
        return data.Gems.FirstOrDefault(g => g.Id == id);
    }
    ...
    public async Task AddGemAsync(AdminGem gem, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var data = await ReadDataInternalAsync(cancellationToken);
            data.Gems.Add(gem);
            await WriteDataInternalAsync(data, cancellationToken);
        }
        finally
        {
            FileLock.Release();
    ...
    public async Task UpdateGemAsync(string id, AdminGem gem, CancellationToken cancellationToken = default)
    {
        ...
        {
            var data = await ReadDataInternalAsync(cancellationToken);
            var existing = data.Gems.FirstOrDefault(g => g.Id == id);
    ...
    {
        var data = await ReadDataAsync(cancellationToken);
        return data.Partners.FirstOrDefault(p => p.Id == id);
    }
    ...
    public async Task UpdatePartnerAsync(string id, AdminPartner partner, CancellationToken cancellationToken = default)
    {
        ...
        {
            var data = await ReadDataInternalAsync(cancellationToken);
            var existing = data.Partners.FirstOrDefault(p => p.Id == id);
            if (existing == null) return;
    ...
    private async Task<AdminDataContainer> ReadDataAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            return await ReadDataInternalAsync(cancellationToken);
        }
        finally
        {
            FileLock.Release();
    ...
    private async Task<AdminDataContainer> ReadDataInternalAsync(CancellationToken cancellationToken)
    {
        if (!File.Exists(_filePath)) return new AdminDataContainer();
    ...
    private async Task WriteDataInternalAsync(AdminDataContainer data, CancellationToken cancellationToken)
    {
    ...
    private static AdminDataContainer GetSeedData()
    {
        return new AdminDataContainer
        {
            Gems =
            [
